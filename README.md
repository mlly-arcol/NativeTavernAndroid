...
└── src/
    ...
    └── NativeTavern.Maui/        # 安卓 App（聊天原型）
        ...
        ├── App.xaml              # 应用资源
        ├── Views/                # ChatPage（聊天）、SettingsPage（设置）
...
1. 角色卡导入（PNG/JSON，`CharacterCardImporter` 已就绪，缺导入 UI）
2. Markdown 渲染（桌面端 `MarkdownViewer` 是 WPF FlowDocument 实现，移动端需换 Skia/自绘方案）
3. 图片附件、群聊、世界书/Persona/Preset 绑定弹窗
4. 第二阶段：本地 GGUF 推理（需要安卓端推理引擎适配，先测内存/发热/耗电）
